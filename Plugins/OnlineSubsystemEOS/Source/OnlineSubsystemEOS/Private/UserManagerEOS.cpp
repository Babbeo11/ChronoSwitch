...
// ChronoSwitch patch: callback type for Device ID creation
...
void FUserManagerEOS::CallEOSConnectLogin(int32 LocalUserNum, const FOnlineAccountCredentials& Credentials)
{
	...
	// ChronoSwitch patch: Device ID (anonymous) login support. The stock plugin never creates
	// the device id and omits the UserLoginInfo display name that EOS requires for this
	// credential type on desktop platforms (ADD_USER_LOGIN_INFO is 0 on Windows).
	// The game passes the player display name in Credentials.Id.
	if (ExternalCredentialType == EOS_EExternalCredentialType::EOS_ECT_DEVICEID_ACCESS_TOKEN)
	{
		EOS_Connect_CreateDeviceIdOptions CreateDeviceIdOptions = { };
		CreateDeviceIdOptions.ApiVersion = 1;
		UE_EOS_CHECK_API_MISMATCH(EOS_CONNECT_CREATEDEVICEID_API_LATEST, 1);
		CreateDeviceIdOptions.DeviceModel = "PC";

		FCreateDeviceIdCallback* CreateDeviceIdCallbackObj = new FCreateDeviceIdCallback(AsWeak());
		CreateDeviceIdCallbackObj->CallbackLambda = [this, LocalUserNum, DisplayName = Credentials.Id](const EOS_Connect_CreateDeviceIdCallbackInfo* Data)
		{
			// DuplicateNotAllowed means the device id already exists, which is fine
			if (Data->ResultCode != EOS_EResult::EOS_Success && Data->ResultCode != EOS_EResult::EOS_DuplicateNotAllowed)
			{
				const FString ErrorString = FString::Printf(TEXT("EOS_Connect_CreateDeviceId(%d) failed with EOS result code (%s)"), LocalUserNum, *LexToString(Data->ResultCode));
				UE_LOG_ONLINE(Warning, TEXT("%s"), *ErrorString);
				RemoveLocalUser(LocalUserNum);
				TriggerOnLoginCompleteDelegates(LocalUserNum, false, *FUniqueNetIdEOS::EmptyId(), ErrorString);
				return;
			...
			DeviceIdCredentials.ApiVersion = 1;
			UE_EOS_CHECK_API_MISMATCH(EOS_CONNECT_CREDENTIALS_API_LATEST, 1);
			DeviceIdCredentials.Type = EOS_EExternalCredentialType::EOS_ECT_DEVICEID_ACCESS_TOKEN;
			DeviceIdCredentials.Token = nullptr; // Must be null for Device ID logins

			const FTCHARToUTF8 DisplayNameUtf8(DisplayName.IsEmpty() ? TEXT("Player") : *DisplayName);
			EOS_Connect_UserLoginInfo DeviceIdUserLoginInfo = { };
			DeviceIdUserLoginInfo.ApiVersion = 2;
			UE_EOS_CHECK_API_MISMATCH(EOS_CONNECT_USERLOGININFO_API_LATEST, 2);
			DeviceIdUserLoginInfo.DisplayName = (const char*)DisplayNameUtf8.Get();

			EOS_Connect_LoginOptions DeviceIdLoginOptions = { };
			DeviceIdLoginOptions.ApiVersion = 2;
			UE_EOS_CHECK_API_MISMATCH(EOS_CONNECT_LOGIN_API_LATEST, 2);
			DeviceIdLoginOptions.Credentials = &DeviceIdCredentials;
			DeviceIdLoginOptions.UserLoginInfo = &DeviceIdUserLoginInfo;

			FConnectLoginCallback* LoginCallbackObj = new FConnectLoginCallback(AsWeak());
			LoginCallbackObj->CallbackLambda = [this, LocalUserNum](const EOS_Connect_LoginCallbackInfo* LoginData)
			{
				if (LoginData->ResultCode == EOS_EResult::EOS_Success)
				{
					FullLoginCallback(LocalUserNum, nullptr, LoginData->LocalUserId);
				}
				else if (LoginData->ResultCode == EOS_EResult::EOS_InvalidUser)
				{
					CreateConnectedLogin(LocalUserNum, nullptr, LoginData->ContinuanceToken);
				}
				else
				{
					const FString ErrorString = FString::Printf(TEXT("Device ID Connect login (%d) failed with EOS result code (%s)"), LocalUserNum, *LexToString(LoginData->ResultCode));
					UE_LOG_ONLINE(Warning, TEXT("%s"), *ErrorString);
					RemoveLocalUser(LocalUserNum);
					TriggerOnLoginCompleteDelegates(LocalUserNum, false, *FUniqueNetIdEOS::EmptyId(), ErrorString);
				}
			};
			EOS_Connect_Login(EOSSubsystem->ConnectHandle, &DeviceIdLoginOptions, LoginCallbackObj, LoginCallbackObj->GetCallbackPtr());
		};
		EOS_Connect_CreateDeviceId(EOSSubsystem->ConnectHandle, &CreateDeviceIdOptions, CreateDeviceIdCallbackObj, CreateDeviceIdCallbackObj->GetCallbackPtr());
		return;
	}
	// ChronoSwitch patch end
